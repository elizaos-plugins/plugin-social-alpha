...
const calculateUserTrustScoreTestCases: TestCase[] = [
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        runtime.createComponent = async (comp: Component) => {
          createdComp = comp;
          return true;
        };
  ...
  },
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        runtime.updateComponent = async (comp: Component) => {
          updatedComp = comp;
        };
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      (runtime.getComponent as any) = async (
        ...
        if (
          ...
          if (profile) {
            return {
              id: createUniqueUuid(agentId, `component-${entityId}`), // Construct a mock component ID
              entityId: entityId,
              agentId: runtime.agentId!,
              worldId: componentWorldId,
              roomId: componentWorldId,
              sourceEntityId: runtime.agentId!,
              type: TRUST_MARKETPLACE_COMPONENT_TYPE,
              createdAt: Date.now(),
              data: profile,

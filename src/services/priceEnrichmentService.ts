  ...
  async enrichAllCalls(
    ...
    for (let i = 0; i < calls.length; i += batchSize) {
      ...
      if (batchNumber % 100 === 0 || batchNumber === totalBatches) {
        ...
        console.log(`⏱️  Time: ${elapsed.toFixed(1)}min elapsed, ~${eta.toFixed(1)}min remaining`);
      }
    ...
    console.log(`💾 Saved ${enrichedCalls.length} enriched calls to ${completeOutputFile}`);
  }

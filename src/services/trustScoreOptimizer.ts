...
export class TrustScoreOptimizer {
  ...
  private generateSuggestions(
    ...
    // Handle empty scores
    if (scores.length === 0) {
      suggestions.push('❌ No scores generated. Check simulation data generation.');
      return suggestions;
    }

    // Check overall accuracy
    if (accuracy.mae > 15) {
      suggestions.push('⚠️ High mean absolute error (>15). Consider adjusting component weights.');
    }
    ...
    if (accuracy.rankingAccuracy < 80) {
      suggestions.push('⚠️ Low ranking accuracy (<80%). Focus on relative scoring improvements.');
    }

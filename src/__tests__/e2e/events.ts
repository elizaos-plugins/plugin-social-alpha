...
const callMessageHandlerForEventsTest = async (
  message: Message,
  ...
  if (!messageHandler)
    throw new Error(
      "Message handler not defined for EventType.MESSAGE_RECEIVED",
    );
  let completed = false;
  const onComplete = () => {
    completed = true;
  ...
  await (messageHandler as any)(payload);
...
const eventHandlerTestCases: TestCase[] = [
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        const { completed } = await callMessageHandlerForEventsTest(
          selfMessage,
          runtime,
        );
        if (!completed)
          throw new Error("onComplete was not called for self-message.");
        if (useModelCalled)
          throw new Error("useModel was called for agent self-message");
  ...
  },
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        if (!completed)
          throw new Error("onComplete was not called for irrelevant message.");
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        const mockService = {
          ...
          calculateUserTrustScore: async () => {
            logger.debug(
              "[MockService EventsTest] calculateUserTrustScore called",
            );
            return 0; // Return a number instead of void
          },
        } as Partial<CommunityInvestorService>;
        (runtime.getService as any) = <T extends Service>(
          st: string,
        ): T | null =>
        ...
        if (useModelCallCount !== 2)
          throw new Error(
            `useModel call count mismatch, expected 2, got ${useModelCallCount}`,
          );
        ...
        if (!createdComponentData)
          throw new Error("User profile component not created");
        ...
        if (
          ...
          throw new Error(
            `Recommendation data mismatch: ${JSON.stringify(rec)}`,
          );
        }
        if (
          !createdTaskInfo ||
          createdTaskInfo.name !== "PROCESS_TRADE_DECISION"
        )
      ...
      try {
        ...
        (runtime.updateComponent as any) = async (comp: Component) => {
          updatedComponentData =
            comp.data as UserTrustProfile; /* Returns Promise<void> */
        };
        (runtime.useModel as any) = async (
          modelType: ModelTypeName,
          params: { prompt: string },
        ...
        const mockService = {
          ...
          calculateUserTrustScore: async () => {
            logger.debug(
              "[MockService EventsTest] calculateUserTrustScore called during update",
            );
            return 0; // Return a number instead of void
          },
        } as Partial<CommunityInvestorService>;
        ...
        if (!updatedComponentData)
          throw new Error("User profile component not updated");
        ...
        if (
          newRec.tokenAddress !== "NEWSELL_ADDR" ||
          newRec.recommendationType !== "SELL"
        )
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        const mockService = {
          ...
          calculateUserTrustScore: async () => {
            logger.debug(
              "[MockService EventsTest] calculateUserTrustScore called for duplicate skip",
            );
            return 0; // Return a number instead of void
          },
        } as Partial<CommunityInvestorService>;
        ...
        if (updateComponentCalled)
          throw new Error(
            "updateComponent was called for a duplicate recommendation.",
          );
        if (createTaskCalled)
          throw new Error(
            "createTask was called for a duplicate recommendation.",
          );
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        const mockService = {
          ...
          calculateUserTrustScore: async () => {
            logger.debug(
              "[MockService EventsTest] calculateUserTrustScore called for token resolution failure",
            );
            return 0; // Return a number instead of void
          },
        } as Partial<CommunityInvestorService>;
        ...
        if (createTaskCalled)
          throw new Error("Task created despite token resolution failure.");
        logger.info(
          "EventHandler: Ignores on token resolution failure - Passed",
        );
      } finally {
        runtime.useModel = originalUseModel;
        runtime.getService = originalGetService;
  ...
  },
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        if (useModelCalled)
          throw new Error("useModel called for muted, unmentioned agent.");
  ...
  },
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        (runtime.getParticipantUserState as any) = async () => "MUTED";
        (runtime.character as any) = {
          ...runtime.character,
          name: "TestAgentEvents",
        };
        (runtime.useModel as any) = async (
          modelType: ModelTypeName,
          params: { prompt: string },
        ...
        if (!relevanceModelCalled)
          throw new Error(
            "Relevance check not called for muted but mentioned agent.",
          );
        logger.info(
          "EventHandler: Muted but mentioned agent processed - Passed",
        );
      } finally {
        runtime.getParticipantUserState = originalGetParticipantUserState;
...
export const eventsTestSuite: TestSuite = {
  name: "Social Alpha Event Handler Tests (ElizaOS Runner Format)",

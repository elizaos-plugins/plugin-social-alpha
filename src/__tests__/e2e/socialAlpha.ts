...
const simulateMessageProcessing = async (
  ...
  logger.error(
    `[E2E DEBUG] About to call message handler with message: "${message.text}"`,
  );
...
const e2eTestCases: TestCase[] = [
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        try {
          /* world setup */ const allWorlds = originalGetWorlds 
        ...
        const profile = (fetchedComponent as any).data as UserTrustProfile;
        ...
        if (
          !createdTaskDetails ||
          (createdTaskDetails as any).name !== "PROCESS_TRADE_DECISION"
        )
          throw new Error("E2E: PROCESS_TRADE_DECISION task not created.");
        if (
          (createdTaskDetails as any).metadata?.recommendationId !== rec.id ||
          (createdTaskDetails as any).metadata?.userId !== testUserId
        )
        ...
        const finalProfile = (finalFetchedComponent as any).data as UserTrustProfile;
  ...
  },
  {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        try {
          const allWorlds = originalGetWorlds ? await originalGetWorlds.call(runtime) : [];
          if (allWorlds && allWorlds.length > 0) testWorldId = allWorlds[0].id;
          else {
            testWorldId = asUUID(uuidv4());
            await originalEnsureWorld.call(runtime, {
    ...
    fn: async (runtime: IAgentRuntime) => {
      ...
      try {
        ...
        const profileBeforeScore =
          (fetchedComponentBeforeScore as any).data as UserTrustProfile;
        ...
        const profileAfterScore =
          (fetchedComponentAfterScore as any).data as UserTrustProfile;
        if (
